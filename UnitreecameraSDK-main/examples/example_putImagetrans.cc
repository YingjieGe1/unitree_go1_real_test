...
int main(int argc, char *argv[])
{
    ...
    while(cam.isOpened())
    {
        ...
        // 保存临时图像
        if (!cv:: imwrite("../tmp/go1_frame_tmp.jpg", left)) {
            std::cerr << "⚠️ Failed to write temporary frame" << std::endl;
            continue;
        }

        // 稍作延时确保写入完成
        cv::waitKey(100);  // 等 100ms（或改成 50）

        // 重命名为目标路径
        if (std::rename("../tmp/go1_frame_tmp.jpg", "../tmp/go1_frame.jpg") != 0) {
            std::cerr << "❌ Failed to rename file" << std::endl;
        } else {
            std::cout << "✅ Frame updated to ../tmp/go1_frame.jpg" << std::endl;
        }
